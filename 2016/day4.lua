function checksum(str, chk)
    ...
    local t = {}
    for i = i1, i2 do
        local chr, count = string.char(i), 0
        for k in string.gmatch(str, chr) do
            count = count + 1
        end
        table.insert(t, {ch = chr, num = count})
    end
    table.sort(t, function (x1, x2) return (x1.num > x2.num) or (x1.num == x2.num and x1.ch < x2.ch) end)
    local checksum
    if #t >= 5 then
        checksum = t[1].ch .. t[2].ch .. t[3].ch .. t[4].ch .. t[5].ch
    end
    if checksum then
        return checksum == chk
    else
...
function analy(str)
    local roomcode = string.match(str, "%b-[")
    local chksum = string.match(str, "%b[]")
