function checkTLS(str)
    ...
    local ti, to = {}, {}
    while i and j do
        table.insert(ti, str:sub(i + 1, j - 1))
        i, j = str:find("%[", i + 1), str:find("%]", j + 1)
    end
    for word in string.gmatch(str1, "%a+") do
        table.insert(to, word)
    end
    local chk = false
    for _, s in pairs(to) do
        for i = 1, #s - 3 do
            local a1, a2 = s:sub(i, i + 1), s:sub(i + 2, i + 3)
            if a1 ~= a2 and a1 == a2:reverse() then
                chk = true
                break
            end
        end   
