...
for i, s in pairs(action) do
    local act, xmin, ymin, xmax, ymax
    if string.find(s,"turn on") then
        act = "turn on"
    elseif string.find(s,"turn of") then
        act = "turn off"
    elseif string.find(s,"toggle") then
        act = "toggle"
    else
        print("error: no action found")
    end
    local range = {}
    for dig in string.gmatch(s, "%d+") do
