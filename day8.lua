...
function getinfo(input,t)
    local space1 = input:find(" ")
    local space2 = input:find(" ",space1 + 1)
    local space3 = input:find(" ",space2 + 1)
    local space4 = input:find(" ",space3 + 1)
    local space5 = input:find(" ",space4 + 1)
    local space6 = input:find(" ",space5 + 1)
    local var1 = input:sub(1,space1 - 1)
    local op = input:sub(space1 + 1, space2 - 1)
    local val1 = tonumber(input:sub(space2 + 1, space3 - 1))
    local var2 = input:sub(space4 + 1,space5 - 1)
    local com = input:sub(space5 + 1,space6 - 1)
    local val2 = tonumber(input:sub(space6 + 1,#input))
    if not t[var1] then t[var1] = 0 end
    if not t[var2] then t[var2] = 0 end
    if comp(t,var2,com,val2) then
        if op == "inc" then
            t[var1] = t[var1] + val1
        elseif op == "dec" then
            t[var1] = t[var1] - val1
...
local instructions = passwords(input)
local t={}
local minp,maxp
for i=1,#instructions do
    local val = getinfo(instructions[i],t)
    if not minp or val<minp then
        minp = val
    end
    if not maxp or val>maxp then
        maxp = val
    end 
end
local min,max
...
print("in the end, the max value registred was")
print("min=" .. min)
print("max=" .. max)
print("during the process, the max value registred was")
print("min=" .. minp)
print("max=" .. maxp)
