...
for word in string.gmatch(input, "%C+") do
    local element = {}
    local data = word:match("%d+-%d+-%d+ %d+:%d+")
    local minus1, minus2 = data:find("-")
    local space1, space2 = word:find(" ")
    local sp1, sp2 = word:find(":")
    minus2 = data:find("-", minus1 + 1)
    element.year = tonumber(data:sub(1, minus1 - 1))
    element.month = tonumber(data:sub(minus1 + 1, minus2 - 1))
    element.day = tonumber(data:sub(minus2 + 1, space1 - 1))
    element.hour = tonumber(data:sub(space2, sp1 - 2))
    element.minute = tonumber(data:sub(sp2))
    local st, en = word:find("] ")
    element.action = word:sub(en + 1)
    table.insert(list, element)
end
function esort(e1, e2)
    if e1.year ~= e2.year then
        return e1.year < e2.year
    elseif e1.month ~= e2.month then
        return e1.month < e2.month
    elseif e1.day ~= e2.day then
        return e1.day < e2.day
    elseif e1.hour ~= e2.hour then
