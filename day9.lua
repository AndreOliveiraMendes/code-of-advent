...
local aux = {}
for i=1,#t do
    local type,action = t[i][1],t[i][2]
    if action == "open" and not lock then
        aux[#aux + 1] = type
        if type == "garbage" then
            lock = true
        end
        local lstr = (type == "garbage") and "<" or "{"
        if not fstr then fstr = lstr else fstr = fstr .. lstr end
