...
function phrase(input)
    ...
    local i = 1
    while spaces do
        local str = string.sub(input,i,spaces-1)
        table.insert(t,str)
        i = spaces + 1
        spaces = string.find(input,string.char(10),i)
    end
    if #input>=i then
        table.insert(t,string.sub(input,i,#input))
    end
    return t
end
function name(input)
    local spaces = string.find(input," ")
    return input:sub(1,spaces-1)
end
...
local phrases=phrase(input)
local output = {}
--insert names
for i=1,#phrases do
    output[i] = {}
    output[i].name = name(phrases[i])
    output[i].weight = weight(phrases[i])
end
--make linked
for i=1,#phrases do
    if linked(phrases[i]) then
        local t = linked(phrases[i])
        local name = t[1]
        local index = checkname(output,t[1])
        local top = {}
        for j=2,#t do
            table.insert(top,t[j])
            local index = checkname(output,t[j])
            output[index].bottom = name
        end
        output[index].linked = top
    end
end
local root = nil
